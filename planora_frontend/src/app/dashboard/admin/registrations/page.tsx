  ...
  const summary = useMemo(() => {
    return {
      ...
      processing: registrations.filter((item) => item.status === "PROCESSING")
        .length,
      banned: registrations.filter((item) => item.status === "BANNED").length,
      ...
      <MainWrapper className="space-y-8">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-2">
              ...
              <p className="text-[var(--color-copy-muted)]">
                Monitor platform registrations, moderate attendees, and process refunds across all events.
              </p>
            </div>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-[28px] border border-[var(--color-border)] bg-white p-6">
            <p className="text-sm text-[var(--color-copy-muted)]">All Registrations</p>
            <p className="mt-3 font-serif text-4xl text-[var(--color-surface-950)]">{summary.total}</p>
          </div>
          <div className="rounded-[28px] border border-[var(--color-border)] bg-white p-6">
            <p className="text-sm text-[var(--color-copy-muted)]">Processing</p>
            <p className="mt-3 font-serif text-4xl text-amber-700">{summary.processing}</p>
          </div>
          <div className="rounded-[28px] border border-[var(--color-border)] bg-white p-6">
            <p className="text-sm text-[var(--color-copy-muted)]">Banned</p>
            <p className="mt-3 font-serif text-4xl text-red-700">{summary.banned}</p>
          </div>
          <div className="rounded-[28px] border border-[var(--color-border)] bg-white p-6">
            <p className="text-sm text-[var(--color-copy-muted)]">Refund Ready</p>
            <p className="mt-3 font-serif text-4xl text-sky-700">{summary.refundReady}</p>
          </div>
        </section>
          ...
          {isLoading ? (
            ...
            <div className="p-16 text-center">
              ...
              <p className="mt-2 text-[var(--color-copy-muted)]">
                Registration activity will appear here once users start joining events.
              </p>
            </div>
          ) : (
